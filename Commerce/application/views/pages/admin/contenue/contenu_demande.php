<div class="card card-warning " style="width: 50%; margin-left: 25%;">
    ...
    <div class="card-body">
        <form action="<?php echo site_url('Admin/validation_demande'); ?>" method="post" id="demande">
            <div class="form-group">
                <label for="departement">Departement</label>
                <select name="departement" class="form-control" id="departement">
                    <?php foreach ($departements as $key => $value) { ?>
                        <option value="<?php echo $value->id_departement ?>"><?php echo $value->libelle ?></option>
                    <?php } ?>
                </select>
            </div>
            <div class="form-group">
...
$(document).ready(function() {
    ...
    $("#materiel").on("change", function() {
        ...
        if(idMateriel != 0){
            $.ajax({
                type: "POST",
                url: "<?php echo site_url("Admin/getProduitsCategorie"); ?>",
